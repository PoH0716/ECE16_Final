...
class BLE:
    ...
    def connect(self, peripheral_mac, max_tries=20):
        ...
        confirmed = False
        while ((not confirmed) and max_tries > 0):
            max_tries -= 1
            content = self.read_lines()
            print(content)
            if "OK+CONNAOK+CONN" in content:
                if ("CONNF" not in content) and ("CONNE" not in content):
                    confirmed = True
            elif "#" in content:
                confirmed = True
            else:
    ...
    def check_connection(self, msg, max_tries=10):
        ...
        while ( ("OK+LOST" in msg) and max_tries > 0):
            max_tries -= 1
    ...
    def read(self):
        ...
        try:
            if self._ser.in_waiting:
                thechar = self._ser.read(1)
                return thechar.decode('utf-8')
            else:
                return ''
